    ...
    *pDevice = nullptr;
    ...
    if (adapter >= 0)
    {
        ...
        if (getDXGIFactory(dxgiFactory.GetAddressOf()))
        {
            ...
            {
                std::cerr << "ERROR: Invalid GPU adapter index: " << adapter;
                return false;
            }
        }
    }
...
bool TexturesOptimizer::decompress()
{
    ...
    if (FAILED(hr))
    {
        std::cerr << "Failed to decompress: " << _name;
        return false;
    }
...
bool TexturesOptimizer::resize(size_t targetWidth, size_t targetHeight)
{
    ...
    if (FAILED(hr))
    {
        std::cerr << "Failed to resize: " + _name;
        return false;
    }
...
bool TexturesOptimizer::generateMipMaps()
{
    ...
    if (_info.mipLevels != 1 && _info.mipLevels != tMips)
    {
        ...
        if (FAILED(hr))
        {
            std::cerr << "Failed to copy texture data to single level (when generating mipmaps) when processing: "
                       << _name;
            return false;
        }
    ...
    if ((_info.width > 1 || _info.height > 1 || _info.depth > 1))
    {
        ...
        if (FAILED(hr))
        {
            std::cerr << "Failed to generate mipmaps when processing: " << _name;
            return false;
        }
...
bool TexturesOptimizer::convertWithoutCompression(const DXGI_FORMAT &format)
{
    if (_info.format != format)
    {
        ...
        if (FAILED(hr))
        {
            std::cerr << "Failed to convert: " + _name;
            return false;
        }
...
bool TexturesOptimizer::convertWithCompression(uint32_t adapter, const DXGI_FORMAT &format)
{
    ...
    if (!createDevice(adapter, _pDevice.GetAddressOf())) {
        std::cerr << "DirectCompute not available!" << std::endl;
    }
    ...
    if (FAILED(hr))
    {
        std::cerr << "Failed to compress: " + _name;
        return false;
    }
